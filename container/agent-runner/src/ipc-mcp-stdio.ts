...
// --- Browser Agent Tools (all groups) ---
// Runs browser-agent on the HOST machine via IPC to avoid container OOM.
...
async function waitForBrowserResult(
  ...
  return { success: false, message: 'Browser agent request timed out' };
}

server.tool(
  'browse_web',
  `Execute a browser task using AI on the HOST machine. The browser agent autonomously
navigates, clicks, types, and extracts data based on your task description.

This tool runs Chromium on the host (not in this container), so it does not
consume container memory. Prefer this over running \`browser-agent\` via Bash.

Use for: web research, reading articles, form filling, data extraction, checking websites.
The browser has access to host Chrome cookies if previously imported via import_cookies.

DO NOT use this for X/Twitter content -- use x_scrape_tweet, x_scrape_profile,
or x_search_tweets instead (they are faster and more reliable for X).`,
  {
    task: z
      .string()
      .describe(
        'What the browser should do (e.g. "Go to example.com and extract the pricing table")',
      ),
    max_steps: z
      .number()
      .default(50)
      .describe('Maximum browser action steps'),
    model: z
      .string()
      .optional()
      .describe('LLM model override (e.g. gpt-5.4)'),
    use_vision: z
      .boolean()
      .default(true)
      .describe('Send screenshots to LLM (disable for sensitive pages)'),
  },
  async (args) => {
    ...
    writeIpcFile(TASKS_DIR, {
      ...
      useVision: args.use_vision,
      groupFolder,
      timestamp: new Date().toISOString(),
    });

    const result = await waitForBrowserResult(requestId, 180000);
    return {
      content: [{ type: 'text' as const, text: result.message }],
      isError: !result.success,
    };
